...
def guess_most_likely(input_):
    guesses = _model.predict_proba(input_)[0]
    return _model.classes_[int(np.argmax(guesses))]


def print_probability(input_):
    guesses = _model.predict_proba(input_)[0]
...
def get_execution_time(func, input_):
...
# Average execution time across multiple runs, each time loading in a random
# numpy array
...
# Calculates the average accuracy for determining a specific label
def get_accuracy_for_label(expected, npy_dir, verbose):
    files = glob.glob(os.path.join(npy_dir, f"{expected}", "*.npy"))
    if not files:
        raise ValueError(
            f"[Metrics] no {expected}-npy files found in: {npy_dir}"
            )
    correct = 0
    for file in files:
...
def print_metric_report():
    ...
    print(
        f"Average execution time across {runs} runs: "
        f"{get_average_execution_time(runs, guess_most_likely, npy_path)} ms\n"
        )
    print("Accuracy")
    for label in labels:
